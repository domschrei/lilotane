...
namespace Names
{
    ...
    std::string to_string(const std::vector<IntPair> &nameIdPairs)
    {
        ...
        {
            out += (first ? "(" : " (") + std::string(left < 0 ? "-" : "") + Names::to_string(std::abs(left)) + "," + std::string(right < 0 ? "-" : "") + Names::to_string(std::abs(right)) + ")";
            first = false;
        }
        return out + ")";
    }
    ...
    std::string to_string(const std::basic_string<int> &str)
    {
        std::string out = "(";
        bool first = true;
        for (const int &id : str)
        {
            out += (first ? "" : " ") + std::string(id < 0 ? "-" : "") + Names::to_string(std::abs(id));
            first = false;
        }
