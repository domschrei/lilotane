...
void Planner::createNextLayer()
{
    ...
    for (_old_pos = 0; _old_pos < oldLayer.size(); _old_pos++)
    {
        ...
        for (size_t offset = 0; offset < maxOffset; offset++)
        {
            ...
            if (_pos > 0)
                _layers[_layer_idx]->at(_pos - 1).clearAfterInstantiation();
        ...
        for (const auto &aSig : *set)
        {
            ...
            for (const Signature &fact : _analysis.getPossibleFactChanges(aSig))
            {
                                    ...
                                    repeatedAction ? EffectMode::DIRECT_NO_QFACT : EffectMode::DIRECT))
                {
                    ...
                    if (left.hasAction(repSig))
                        actionsToRemove.insert(repSig);
...
void Planner::addPreconditionsAndConstraints(const USignature &op, const SigSet &preconditions, bool isRepetition)
{
    ...
    for (const Signature &fact : preconditions)
    {
        ...
        if (cOpt)
            newPos.addSubstitutionConstraint(constrOp, std::move(cOpt.value()));
    }
    if (!isRepetition)
        addQConstantTypeConstraints(op);

    if (!newPos.getSubstitutionConstraints().count(op))
        return;
...
std::optional<SubstitutionConstraint> Planner::addPrecondition(const USignature &op, const Signature &fact, bool addQFact)
{
    ...
    std::basic_string<int> sorts = _htn.getOpSortsForCondition(factAbs, op);
    std::basic_string<int> sortedArgIndices = SubstitutionConstraint::getSortedSubstitutedArgIndices(_htn, factAbs._args, sorts);
    std::vector<int> involvedQConsts(sortedArgIndices.size());
    for (size_t i = 0; i < sortedArgIndices.size(); i++)
        involvedQConsts[i] = factAbs._args[sortedArgIndices[i]];
...
bool Planner::addEffect(const USignature &opSig, const Signature &fact, EffectMode mode)
{
    ...
    std::basic_string<int> sorts = _htn.getOpSortsForCondition(factAbs, opSig);
    std::basic_string<int> sortedArgIndices = SubstitutionConstraint::getSortedSubstitutedArgIndices(_htn, factAbs._args, sorts);
    ...
    if (isConstrained)
    {
        ...
        {
            if (c.getInvolvedQConstants() == involvedQConsts)
                fittingConstrs.push_back(&c);
    ...
    if (!staticallyResolvable && mode == DIRECT)
        pos.addQFact(factAbs);
...
void Planner::propagateActions(size_t offset)
{
    ...
    for (const auto &aSig : above.getActions())
    {
        ...
        bool valid = _analysis.hasValidPreconditions(a.getPreconditions()) && _analysis.hasValidPreconditions(a.getExtraPreconditions());
    ...
    assert(above.getActions().size() == numActionsBefore - actionsToPrune.size() || Log::e("%i != %i-%i\n", above.getActions().size(), numActionsBefore, actionsToPrune.size()));
...
std::vector<USignature> Planner::instantiateAllActionsOfTask(const USignature &task)
{
    ...
    for (USignature &sig : _instantiator.getApplicableInstantiations(_htn.toAction(task._name_id, task._args)))
    {
        ...
        // Check validity
        if (!_htn.isFullyGround(action.getSignature()))
            continue;
        if (!_htn.hasConsistentlyTypedArgs(sig))
            continue;
...
std::vector<USignature> Planner::instantiateAllReductionsOfTask(const USignature &task)
{
    ...
    if (!_htn.hasReductions(task._name_id))
        return result;
    ...
    for (int redId : _htn.getReductionIdsOfTaskId(task._name_id))
    {
        ...
        for (const Substitution &s : subs)
        {
            ...
            if (!_htn.hasConsistentlyTypedArgs(origSig))
                continue;
            ...
            for (USignature &red : _instantiator.getApplicableInstantiations(rSub))
            {
                ...
                if (rOpt)
                    result.push_back(rOpt.value().getSignature());
            }
        }
    }
    return result;
...
std::optional<Reduction> Planner::createValidReduction(const USignature &sig, const USignature &task)
{
    ...
    // Check validity
    bool isValid = true;
    if (task._name_id >= 0 && red.getTaskSignature() != task)
        isValid = false;
    else if (!_htn.isFullyGround(red.getSignature()))
        isValid = false;
    else if (!_htn.hasConsistentlyTypedArgs(red.getSignature()))
        isValid = false;
    ...
    for (const auto &set : ops)
    {
        for (const auto &aSig : *set)
        {
            ...
            for (const Signature &eff : pfc)
            {
                ...
                {
                    std::basic_string<int> sorts = _htn.getOpSortsForCondition(eff._usig, aSig);
...
void Planner::initializeFact(Position &newPos, const USignature &fact)
{
    ...
    if (_analysis.isInitialized(fact))
        return;

    _analysis.addInitializedFact(fact);

    if (_analysis.isReachable(fact, /*negated=*/true))
        newPos.addFalseFact(fact);
    else
        newPos.addTrueFact(fact);
}
...
void Planner::clearDonePositions(int offset)
{
    ...
    if (_pos == 0 && _layer_idx > 0)
    {
        ...
        positionToClearLeft = &_layers.at(_layer_idx)->at(_pos - 1);
    ...
    if (_old_pos == 0)
    {
        ...
        if (_layer_idx > 1)
            positionToClearAbove = &_layers.at(_layer_idx - 2)->at(_layers.at(_layer_idx - 2)->size() - 1);
    }
    else
    {
...
void Planner::checkTermination()
{
    ...
    else if (_time_at_first_plan == 0 && _init_plan_time_limit > 0 && Timer::elapsedSeconds() > _init_plan_time_limit)
    {
