...
private:
    ...
    std::basic_string<int> _task_args;
...
public:
    ...
    Reduction(int nameId, const std::basic_string<int> &args, const USignature &task);
    Reduction(int nameId, const std::basic_string<int> &args, USignature &&task);
    ...
    const std::basic_string<int> &getTaskArguments() const;
