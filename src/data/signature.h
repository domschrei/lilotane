...
struct TypeConstraint
{
    ...
    TypeConstraint(int qconstant, bool sign, const std::vector<int> &constants) : qconstant(qconstant), sign(sign), constants(constants) {}
    TypeConstraint(int qconstant, bool sign, std::vector<int> &&constants) : qconstant(qconstant), sign(sign), constants(constants) {}
...
struct USignature
{
    ...
    int _name_id = -1;
    std::basic_string<int> _args;

    USignature();
    USignature(int nameId, const std::basic_string<int> &args);
    USignature(int nameId, std::basic_string<int> &&args);
...
struct Signature
{
    ...
    Signature();
    Signature(int nameId, const std::basic_string<int> &args, bool negated = false);
    Signature(int nameId, std::basic_string<int> &&args, bool negated = false);
...
struct SigVecHasher
{
    ...
    {
        size_t hash = s.size();
        for (const Signature &sig : s)
            hash_combine(hash, _sig_hasher(sig));
        return hash;
    }
};
...
namespace Sig
{
    const static USignature NONE_SIG = USignature(-1, {});
