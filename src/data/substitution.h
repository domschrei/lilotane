...
public:
    ...
    Substitution(const std::basic_string<int> &src, const std::basic_string<int> &dest);
    ...
    static std::vector<Substitution> getAll(const std::basic_string<int> &src, const std::basic_string<int> &dest);
    ...
    inline int &operator[](const int &key)
    {
        ...
        while (it != _entries.end())
        {
            ...
            if (nextIt == _entries.end() || nextIt->first > key)
                break;
    ...
    {
        auto it = _entries.begin();
        while (it != _entries.end() && it->first != key)
            ++it;
        return it->second;
    }
