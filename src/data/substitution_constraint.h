...
public:
    ...
    SubstitutionConstraint(const SubstitutionConstraint &other) : _involved_q_consts(other._involved_q_consts),
    ...
    SubstitutionConstraint(SubstitutionConstraint &&other) : _involved_q_consts(other._involved_q_consts),
    ...
    {
        if (_polarity != other._polarity)
            return false;
        if (_polarity == UNDECIDED)
            return false;
    ...
    {
        if (p == ANY_VALID)
            return _valid_substitutions.encode();
        if (p == NO_INVALID)
    ...
    static std::basic_string<int> getSortedSubstitutedArgIndices(HtnInstance &htn, const std::basic_string<int> &qargs, const std::basic_string<int> &sorts)
    {

        // Collect indices of arguments which will be substituted
        std::basic_string<int> argIndices;
        for (size_t i = 0; i < qargs.size(); i++)
        {
            if (htn.isQConstant(qargs[i]))
        ...
        std::sort(argIndices.begin(), argIndices.end(),
                  [&](int i, int j)
                  { return htn.getConstantsOfSort(sorts[i]).size() < htn.getConstantsOfSort(sorts[j]).size(); });
        return argIndices;
    }

    static std::vector<IntPair> decodingToPath(const std::basic_string<int> &qArgs, const std::basic_string<int> &decArgs, const std::basic_string<int> &sortedIndices)
    {
