...
HtnInstance::HtnInstance(Parameters &params) : _params(params), _p(*parse(params.getDomainFilename(), params.getProblemFilename())),
...
void HtnInstance::printStatistics()
{
    ...
    for (const auto &[nameId, r] : _methods)
    {
        ...
        if (!numMethodsMatchingTaskNameId.count(taskNameId))
            ...
            numMethodsMatchingTaskNameId[taskNameId]++;
    }
...
void HtnInstance::primitivizeSimpleReductions()
{
    ...
    for (const auto &entry : _methods)
    {
        ...
        for (const auto &pre : red.getPreconditions())
            _operators[id].addPrecondition(pre);
        for (const auto &pre : red.getExtraPreconditions())
            _operators[id].addExtraPrecondition(pre);
        for (const auto &pre : childAct.getPreconditions())
            _operators[id].addPrecondition(pre);
        for (const auto &pre : childAct.getExtraPreconditions())
            _operators[id].addExtraPrecondition(pre);
        for (const auto &eff : childAct.getEffects())
            _operators[id].addEffect(eff);
...
std::basic_string<int> HtnInstance::convertArguments(int predNameId, const std::vector<std::pair<string, string>> &vars)
{
    std::basic_string<int> args;
...
std::basic_string<int> HtnInstance::convertArguments(int predNameId, const std::vector<std::string> &vars)
{
    std::basic_string<int> args;
    for (const auto &var : vars)
    {
...
USigSet HtnInstance::getInitState()
{
    ...
    for (int eqPredId : _equality_predicates)
    {
        ...
        const std::basic_string<int> &sorts = getSorts(eqPredId);
...
Action HtnInstance::getGoalAction()
{
    ...
    Action goalAction(nameId("<goal_action>"), {});
...
Reduction &HtnInstance::createReduction(method &method)
{
    ...
    std::basic_string<int> args = convertArguments(id, method.vars);

    int taskId = nameId(method.at);
    _task_id_to_reduction_ids[taskId];
    _task_id_to_reduction_ids[taskId].push_back(id);
    {
        std::basic_string<int> taskArgs = convertArguments(id, method.atargs);
    ...
    for (const plan_step &st : method.ps)
    {
        ...
        if (subtaskName.rfind(method_precondition_action_name) != std::string::npos)
        {
            ...
            for (const literal &lit : precTask.prec)
                condLiterals.push_back(lit);
            for (const literal &lit : precTask.constraints)
                condLiterals.push_back(lit);
...
Action &HtnInstance::createAction(const task &task)
{
    ...
    std::basic_string<int> args = convertArguments(id, task.vars);
    ...
    for (const literal &lit : lits)
    {
        ...
        if (lit.predicate == dummy_equal_literal)
        {
            ...
            if (!_signature_sorts_table.count(newPredId))
            {
                ...
                _signature_sorts_table[newPredId] = std::basic_string<int>(2, eqSort);
                _equality_predicates.insert(newPredId);
                _predicate_ids.insert(newPredId);
            }

            // Add as a precondition
            std::basic_string<int> args(2, 0);
...
HtnOp &HtnInstance::getOp(const USignature &opSig)
{
    ...
    if (it != _operators.end())
        return (HtnOp &)it->second;
    else
        return (HtnOp &)_methods.at(opSig._name_id);
}
...
{
    std::basic_string<int> newArgs = replaceVariablesWithQConstants((const HtnOp &)a, opArgDomains, layerIdx, pos);
...
{
    std::basic_string<int> newArgs = replaceVariablesWithQConstants((const HtnOp &)red, opArgDomains, layerIdx, pos);
    if (newArgs.size() == 1 && newArgs[0] == -1)
    {
        // No valid substitution.
        return red;
...
std::basic_string<int> HtnInstance::replaceVariablesWithQConstants(const HtnOp &op,
                                                                   const std::vector<FlatHashSet<int>> &domainPerVariable, int layerIdx, int pos)
{

    if (op.getArguments().empty())
        return {};
    std::basic_string<int> vecFailure(1, -1);

    std::basic_string<int> args = op.getArguments();
    std::basic_string<int> varargIndices;
    for (size_t i = 0; i < args.size(); i++)
    {
        const int &arg = args[i];
        if (isVariable(arg))
            varargIndices.push_back(i);
    }
    if (domainPerVariable.empty())
        return vecFailure;

    // Assemble new operator arguments
    FlatHashMap<int, int> numIntroducedQConstsPerType;
    NodeHashMap<int, std::basic_string<int>> domainsPerQConst;
    for (int i : varargIndices)
    {
        ...
        else
        {
            ...
            std::basic_string<int> domainVec(domain.begin(), domain.end());
            std::stringstream domainHash;
            domainHash << std::hex << USignatureHasher()(USignature(primarySort, domainVec));
            std::string qConstName = "Q_" + std::to_string(layerIdx) + "," + std::to_string(pos) + "_" + _name_back_table[primarySort] + ":" + std::to_string(sortCounter) + "_" + domainHash.str() + (_share_q_constants ? std::string() : "_#" + std::to_string(_q_constants_with_origin.size()));
...
void HtnInstance::initQConstantSorts(int id, const FlatHashSet<int> &domain)
{
    ...
    for (const auto &sortPair : _p.sorts)
        qConstSorts.insert(nameId(sortPair.first));
    ...
    for (int c : _constants_by_sort[newSortId])
    {
        ...
        {
            if (std::find(_constants_by_sort[qsort].begin(), _constants_by_sort[qsort].end(), c) == _constants_by_sort[qsort].end())
            {
                sortsToRemove.push_back(qsort);
            }
        }
        for (int remSort : sortsToRemove)
            qConstSorts.erase(remSort);
    }
...
std::vector<std::basic_string<int>> HtnInstance::getEligibleArgs(const USignature &qSig,
                                                                 const std::basic_string<int> &restrictiveSorts)
{

    std::vector<std::basic_string<int>> eligibleArgs;
...
ArgIterator HtnInstance::decodeObjects(const USignature &qSig, std::vector<std::basic_string<int>> eligibleArgs)
{
    return ArgIterator(qSig._name_id, std::move(eligibleArgs));
}

SampleArgIterator HtnInstance::decodeObjects(const USignature &qSig, std::vector<std::basic_string<int>> eligibleArgs, size_t numSamples)
{
    return SampleArgIterator(qSig._name_id, std::move(eligibleArgs), numSamples);
}

const std::basic_string<int> &HtnInstance::getSorts(int nameId) const
{
...
std::basic_string<int> HtnInstance::getOpSortsForCondition(const USignature &sig, const USignature &op)
{
    std::basic_string<int> sigSorts(sig._args.size(), 0);
...
std::basic_string<int> HtnInstance::popOperationDependentDomainOfQConstant(int qconst, const USignature &op)
{
    ...
    std::basic_string<int> domain = it2->second;
    if (opDomains.size() == 1)
    {
...
Action HtnInstance::toAction(int actionName, const std::basic_string<int> &args) const
{
    const auto &op = _operators.at(actionName);
    return op.substitute(Substitution(op.getArguments(), args));
}

Reduction HtnInstance::toReduction(int reductionName, const std::basic_string<int> &args) const
{
...
USignature HtnInstance::getNormalizedLifted(const USignature &opSig, std::basic_string<int> &placeholderArgs)
{
